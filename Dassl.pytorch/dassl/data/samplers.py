...
from .episodic_sampler import EpisodicSampler
...
def build_sampler(
    ...
    elif sampler_type == "EpisodicSampler":
        n_way = getattr(cfg, 'META_N_WAY', 5) if cfg else 5
        k_support = getattr(cfg, 'META_K_SUPPORT', 1) if cfg else 1
        k_query = getattr(cfg, 'META_K_QUERY', 3) if cfg else 3
        n_episodes = getattr(cfg, 'META_N_EPISODES', 100) if cfg else 100
        return EpisodicSampler(data_source, n_way, k_support, k_query, n_episodes)
