...
class TrainerBase:
    ...
    def load_model(self, directory, epoch=None):
        ...
        for name in names:
            ...
            print(
                f"Load {model_path} to {name} (epoch={epoch}, val_result={val_result:.2f})"
            )
    ...
    def train(self, start_epoch, max_epoch):
        ...
        progress_path = osp.join(self.output_dir, "progress.log")
        self._progress_fh = open(progress_path, "w")
        self._progress_fh.write(f"[START] {datetime.datetime.now().isoformat()}\n")
        self._progress_fh.flush()

        self.before_train()
        for self.epoch in range(self.start_epoch, self.max_epoch):
            self._progress_fh.write(f"\n[EPOCH {self.epoch+1}/{self.max_epoch}] {datetime.datetime.now().strftime('%H:%M:%S')}\n")
            self._progress_fh.flush()
            self.before_epoch()
            self.run_epoch()
            self.after_epoch()
    ...
    def _log_progress(self, msg):
        if hasattr(self, "_progress_fh") and self._progress_fh:
            self._progress_fh.write(f"  {msg}\n")
            self._progress_fh.flush()
        sys.stdout.flush()
