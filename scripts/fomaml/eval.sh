#!/bin/bash
# FOMAML Novel Class Evaluation Script
# Usage: bash scripts/fomaml/eval.sh --dataset stanford_cars --seed 1 --load-epoch 100
set -e
...
CMD="$PYTHON train.py \
    ...
    --model-dir output/fomaml/${DATASET}/seed${SEED}/ \
    --load-epoch ${EPOCH} --eval-only \
    DATASET.SUBSAMPLE_CLASSES new \
    TRAINER.COOP.N_CTX 2 \
    TRAINER.ATPROMPT.USE_ATPROMPT True"

echo "[FOMAML EVAL]  dataset=$DATASET seed=$SEED epoch=$EPOCH"
eval "$CMD" 2>&1
