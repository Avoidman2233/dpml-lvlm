#!/usr/bin/env python3
"""5-way episodic evaluation for ProtoATP-CoCoOp checkpoint."""
...
def make_cfg(data_dir, dataset, output_dir=""):
    cfg = get_cfg_default()
    ...
    cfg.OUTPUT_DIR = output_dir or f"output/protoatp/{dataset}/eval_cocoop"
    cfg.TRAINER.COOP.N_CTX = 2
    cfg.TRAINER.COCOOP.N_CTX = 2
    ...
    cfg.TRAINER.COCOOP.N_CTX = cfg.TRAINER.COOP.N_CTX
    return cfg


def episodic_eval(model, cfg, n_way, k_support, k_query, n_episodes, seed, device):
    random.seed(seed)
    np.random.seed(seed)
    ...
    with torch.no_grad():
        for support_idxs, query_idxs in tqdm(sampler, desc=f"Eval {n_way}w{k_support}s"):
            ...
            conditional_prompts = model.prompt_learner(visual_features.type(dtype))
            per_img_prompts = conditional_prompts[
                torch.arange(len(support_label), device=device),
                support_label
            ]
            per_img_tokens = model.tokenized_prompts[support_label.cpu()].to(device)
            text_features_all = model.text_encoder(per_img_prompts, per_img_tokens)
            text_features_all = F.normalize(text_features_all, dim=-1)
            text_protos = []
            for c in unique_labels:
                mask = support_label == c
                text_protos.append(text_features_all[mask].mean(0))
...
if __name__ == "__main__":
    ...
    clip_model = trainers.cocoop_atp.load_clip_to_cpu(cfg)
    model = trainers.cocoop_atp.CustomCLIP(cfg, classnames, clip_model)
    model.to(device)
