#!/bin/bash
# DL-MPT-Align(CoOp+ATP) Sweep — 8 datasets × 3 seeds = 24 runs
# Usage: bash scripts/dlmpt_align/sweep.sh [--dry-run]
set -e
...
SEEDS=(1 2 3)
LAMBDA=0.2
LOG_FILE="$PROJ/output/dlmpt_align_coop_atp/sweep.log"
...
for DS in "${DATASETS[@]}"; do
    ...
    for S in "${SEEDS[@]}"; do
        CFG="${DS_CFG[$DS]}"
        OUT="output/dlmpt_align_coop_atp/${DS}/seed${S}"
        CMD="CUDA_VISIBLE_DEVICES=0 $PY train.py \
            --root $DATA --seed $S --trainer DLMPT_Align_CoOp_ATP \
