#!/bin/bash
# DL-MPT-Adaptive Novel Class Evaluation Script
# Usage: bash scripts/dlmpt_adaptive/eval.sh --dataset stanford_cars --seed 1 --load-epoch 25
set -e
...
CMD="$PYTHON train.py \
    ...
    --model-dir output/dlmpt_adaptive/${DATASET}/seed${SEED}/ \
    --load-epoch ${EPOCH} --eval-only \
    DATASET.SUBSAMPLE_CLASSES new \
    TRAINER.COOP.N_CTX 2 \
    TRAINER.ATPROMPT.USE_ATPROMPT True"

echo "[DL-MPT-ADAPTIVE EVAL]  dataset=$DATASET seed=$SEED epoch=$EPOCH"
eval "$CMD" 2>&1
